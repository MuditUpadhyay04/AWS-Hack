...
// this will POST the user's notes (raw + structured) to it. No component code
// changes either way.

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL;

export async function fetchRoadmap(notes: NotesDocument): Promise<Roadmap> {
  ...
  const res = await fetch(`${API_BASE_URL}/roadmap`, {
    ...
    body: JSON.stringify({ notes: notes.raw, structured: notes.sections }),
  });
